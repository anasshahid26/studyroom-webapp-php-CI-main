...
if(isset($_POST['start_button'])){
    ...
    redirect('/student/online-session/', 'refresh');
}



?>
<div id="videos" class="sss">
    <div id="publisher" class="student_publisher"></div>
    <div id="subscriber" class="student_subscriber"></div>
</div>
  ...
  //  initializeSession()
    ...
    function initializeSession() {
        ...
        session.on('streamCreated', function(event) {
           ...
           // console.log(event.stream.element);
            if(event.stream.name =="student"){
console.log("st");
                session.subscribe(event.stream, 'subscriber', {
                    insertMode: 'append',
                    hasVideo:'false',
                    data:'this is student',
                    width: '100%',
                    height: '100%'
                }, handleError);
            }else{
...
<script>
function emit() {
    var myInnerHtml = document.getElementsByClassName("videos").innerHTML;
    alert(myInnerHtml);
}
