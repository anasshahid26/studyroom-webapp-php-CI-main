...
class Student extends MY_Controller
{
    ...
    function book_tutor()
    {
        ...
        if($ref > 0) {
            ...
            $abc = $_POST['gateway_id'];
            $this->prepare_flashmessage(get_languageword('your_slot_with_the_tutor_booked_successfully_Once_tutor_approved_your_booking and_initiated_the_session_you_can_start_the_course_on_the_booked_date'." and you have selected  " . $abc .$fee) , 0);
            //.$course_slug.$preferred_location.
            //$this->prepare_flashmessage(get_languageword('please enter the above deail'));
            //  $this->site url->payment->get_additional_content;
    ...
    function student_online_session()
    {
        if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_student()) {
            $this->prepare_flashmessage(get_languageword('You dont have permission to access this page'), 1);
            redirect('auth/login', 'refresh');
        ...
        $crud->set_relation('tutor_id',TBL_USERS, 'username');
        $crud->set_relation('course_id',TBL_TUTOR_COURSES, 'course_ids');
        ...
        //List Table Columns
        $crud->columns('course_id', 'tutor_id', 'course_duration', 'fee','content', 'start_date', 'time_slot','onlinee');
        ...
        //Display Alias Names
        //  $crud->display_as('course_name',get_languageword('course_name'));
        $crud->display_as('tutor_id',get_languageword('tutor_id'));
        $crud->display_as('fee',get_languageword('fee').' ('.get_languageword('in_credits').')');
        $crud->display_as('admin_commission',get_languageword('admin_commission_percentage_in_credits'));
        $crud->display_as('per_credit_value',get_languageword('per_credit_value')." (".get_system_settings('currency_symbol').")");
        $crud->display_as('start_date',get_languageword('preferred_commence_date'));
