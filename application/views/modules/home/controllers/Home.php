...
class Home extends MY_Controller 
{
	...
	function index()
	{
		...
		//Location Options
		$locations = $this->home_model->get_categories(array('child' => true));
		$location_opts[''] = get_languageword('select_category');
		//print_r($locations);
		if(!empty($locations)) {
			foreach ($locations as $key => $value) {
				if($show_records_count_in_search_filters == "Yes") {
                //  print_r(array('course_slug'=>$value->slug));
					$avail_records_cnt = " (".count($this->home_model->get_tutors(array('course_slug'=>$value->slug))).")";
				}
			 	$location_opts[$value->slug] = $value->name.$avail_records_cnt;
		...
		$course_opts[''] = get_languageword('Subjects');
		if(!empty($courses)) {
		...
		if($this->input->post()) {
			...
			if($this->form_validation->run() == TRUE) {
				...
				if($ref) {
                    ...
                    $this->load->student->controller['personal_info'];
                    $inputdata['gender'] = $this->input->post('gender');
