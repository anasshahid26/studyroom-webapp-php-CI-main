...
class Tutor extends MY_Controller 
{
	...
	function get_credit()
    {
    	...
    	$get_data = $this->tutor_model->get_credit_course($course_id);
if($course_id<1){
    $durationType = "2 years";
    $durationValue = "3 hours";
}
else{
    $durationType = "12 years";
    $durationValue = "13 hours";
}
    	$this->data['credit']= $get_data;
        $this->data['durationType']= $durationType;
        $this->data['durationValue']= $durationValue;
...
//Setting values for tabel columns
            $data = array(
                'is_certificates' => $this->input->post('dname'),

            );
//Transfering data to Model
            $this->insert_model->get_is_cert($data);
...
//Loading View
		...
		$get_is_cert = $this->tutor_model->get_is_cert($user_id);
		//print_r( $user_id);
		...
		$this->data['inst_tutor_dashboard']	= $inst_tutor_dashboard;
		$profile = getUserRec();
		$this->data['user_idd'] = $get_is_cert;
		$this->data['pagetitle'] 	= get_languageword('dashboard');
		$this->data['activemenu'] 	= "dashboard";
		$this->data['activesubmenu'] = "dashboard";
	...
	function manage_subjects()
	{
		...
		$this->data['activemenu'] 	= "manage";manage-courses
	...
	function manage_courses()
	{
		...
		//List Table Columns
		$crud->columns('course_id','course_duration','course_duration','fee','content','time_slots', 'days_off');
		...
		$crud->edit_fields('tutor_id', 'course_id','duration_type','duration_value', 'fee','credit', 'per_credit_value', 'content', 'time_slots', 'days_off', 'status', 'sort_order', 'updated_at');
		...
		$crud->field_type('per_credit_value', 'hidden', get_system_settings('credit'));
		...
		if($crud_state == "edit" || $crud_state == "read") {
			...
			if($tutor_id != $user_id) {
    			...
    			var_dump(URL_TUTOR_MANAGE_COURSES);
			}
	...
	function call_back_set_course_dropdown($val)
	{
        ...
        $course_opts_id[''] = "select_class_id";
      //  echo get_languageword('select_course');

		if( ! empty( $courses ) ) {
			foreach ($courses as $key => $value) {
				$course_opts[$value->id] = $value->name;
                $course_opts_id[$value->id] = $value->id;
		...
		return form_dropdown('course_id', $course_opts, $val, 'id="course_id" class="chosen-select" name= ');
	}
    ...
    function interview()
    {
        ...
        $this->data['pagetitle'] = get_languageword('Interview');
        $this->data['activemenu'] 	= "manage";
        $this->data['activesubmenu'] = "interview";
        $this->data['content'] 		= "interview";
        $this->_render_page('template/site/tutor-template', $this->data);
    }
    ...
    function online_session()
    {
        ...
        //List Table Columns
        $crud->columns('course_id','course_duration','fee','content','time_slots', 'days_off','online');
        ...
        //From Validations
        $crud->required_fields(array('course_id','duration_value','duration_type', 'fee', 'content', 'time_slots','online', 'sort_order'));
        $crud->set_rules('fee',get_languageword('fee'),'integer');
      ...
      $crud->field_type('days_off', 'multiselect', array('SUN' => 'SUN', 'MON' => 'MON', 'TUE' => 'TUE', 'WED' => 'WED', 'THU' => 'THU', 'FRI' => 'FRI', 'SAT' => 'SAT'));
       ...
       // Authenticate whether Tutor editing/viewing his records only
        ...
        $this->data['activemenu'] 	= 'online_session';
        // $this->data['activesubmenu'] 	= 'courses';
        $this->data['pagetitle'] = "Online Sesssion";
        $this->data['grocery_output'] = $output;
        $this->data['grocery'] = TRUE;
        $this->grocery_output($this->data);
    }
    ...
    function start_online_session()
    {
       ...
       /* $session_name = "courseid".$_GET['course_id'];
        $dataa = array(
...
//Transfering data to Model
      //  $this->tutor_model->session_insert($dataa);

        $this->data['profile'] = getUserRec();
        $this->data['activemenu'] 	= "online_session";
        // $this->data['activesubmenu'] = "update_contact_info";
        $this->data['content'] 		= 'online_session';
        $this->data['pagetitle'] = "online session";
        $this->_render_page('template/site/tutor-template', $this->data);
    }



    function start_online_chat()
    {
        $user_id = $this->ion_auth->get_user_id();
        $get_is_cert = $this->tutor_model->get_is_cert($user_id);
        $this->data['user_idd'] = $get_is_cert;
...
//Transfering data to Model
        //  $this->tutor_model->session_insert($dataa);
        $user_id = $this->ion_auth->get_user_id();
        $get_is_cert = $this->base_model->get_is_cert($user_id);

        $this->data['user_details'] = $get_is_cert;
        $this->data['profile'] = getUserRec();
        $this->data['activemenu'] 	= "chat";
        // $this->data['activesubmenu'] = "update_contact_info";
        $this->data['content'] 		= 'chat';
        $this->data['pagetitle'] = "Online Chat";
        // $this->_render_page('template/site/tutor-template', $this->data);
        $this->_render_page('template/site/tutor-template', $this->data);
    }
