...
class Home_Model extends CI_Model  
{
    ...
    function get_catid_by_slug($cat_slug)
    {
        if(empty($cat_slug))
            return 0;

        $column_to_select = 'id';
        if(is_array($cat_slug) || $cat_slug instanceof Traversable) {

            $column_to_select = 'GROUP_CONCAT(id) AS id';
        }
        $result_set = $this->db->select($column_to_select)
            ->where_in('slug', $cat_slug)
            ->get(TBL_CATEGORIES);
	...
	function get_categories($params = array())
	{
        ...
        if(!empty($params['start']) && !empty($params['limit']) && $params['start'] >= 1 && $params['limit'] >= 1) {

            $limit_cond = ' LIMIT '.$params['start'].', '.$params['limit'];

        } elseif(empty($params['start']) && !empty($params['limit']) && $params['limit'] >= 1){
...
//print_r($params);
        ...
        if(!empty($params['location_slug'])) {
        	...
        	$location_id = $this->get_catid_by_slug($params['location_slug']);

        	if(empty($location_id))
        		return FALSE;

        	$location_tbl_join = " INNER JOIN ".TBL_TUTOR_COURSES." tl ON tl.tutor_id=u.id ";
        	$location_cond = " AND tl.course_id IN (".$course_id.") ";
        }
